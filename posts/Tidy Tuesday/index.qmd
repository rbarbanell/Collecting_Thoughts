...
......)
```{r}
#how many gendered females vs males in each season
...
ggplot(alone, aes(days_lasted, medically_evacuated ))+
  geom_col()+
  facet_wrap(~gender)
...
legend="right",legend.title = "Gender",ggtheme = theme_minimal()) +
  labs(
...
legend="right",legend.title = "Gender",ggtheme = theme_minimal())+ labs(
...
alone%>%
ggplot(aes(as.factor(result),days_lasted, fill=gender))+
  geom_col()+
  facet_wrap(~gender)+
    ylim(0, 110)
